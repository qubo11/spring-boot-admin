;(function () {
    var DataTableEditor = function (ele, table, options) {
        ...
        var inputFileHtml='<span class="file-wrapper"><input type="file" name="files" class="hidden"/></span>'
        var inputFileWrapperHtml='<span class="file-select-wrapper"><input type="text" data-id="${id}" name="file-select" value="${name}" readonly="readonly"/></span>'
        var inputFileShowHtml='<span data-id="${id}">${name}</span>';
        ...
        function updateBindRow(id) {
            ...
            var rowNode=row.data(data).node();
            initFileUpload(rowNode);
            setButtonListener();
            recordData.isEdited = true;
            recordData.editedRow = id;
            ...
            for (var i = 0; i < columnCount; i++) {
                if (i == columnCount - 1) {
                    ...
                    if (type == 'text') {
                    	...
                    	var mInputFileWrapperHtml = inputFileWrapperHtml;
                    	if(mData){
                        	var fileId=$(mData[i]).data("id");
                        	var fileName=$(mData[i]).text();
                        	if(fileId){
                        		mInputFileWrapperHtml=mInputFileWrapperHtml.replace("${id}",fileId);
                        	}else{
                        		mInputFileWrapperHtml=mInputFileWrapperHtml.replace("${id}","");
                        	}
                        	if(fileName){
                        		mInputFileWrapperHtml = mInputFileWrapperHtml.replace("${name}",fileName);
                        	}else{
                        		mInputFileWrapperHtml = mInputFileWrapperHtml.replace("${name}","");
                        	}
                    	}else{
                    		mInputFileWrapperHtml=mInputFileWrapperHtml.replace("${id}","");
                        	mInputFileWrapperHtml = mInputFileWrapperHtml.replace("${name}","");
                    	}
                    	data[i]=mInputFileWrapperHtml+inputFileHtml;
                    }
                }
            }
            return data;
        ...
        function initFileUpload(rowNode){
        	$(rowNode).find("input[name='files']").each(function(){
        		var $this=$(this);
        		var $fileSelect=$this.parent().prev().find("input");
        		$fileSelect.click(function(){
        			$this.click();
        			$this.fileupload({
            			url : options.urls.file,
            			dataType : "json",
            			autoUpload : true,
            			formData : options.fileData,
            			done:function(e, data){
            				if(data.result.result){
            					$fileSelect.data("id",data.result.id);
            					$.each(data.files, function(index, file) {
            						$fileSelect.val(file.name);
            	    			});
            				}
            				if(mSaveFileCallback){
            					mSaveFileCallback(data.result);
            				}
            			}
            		});
        		});
            ...
            if (isEdited) {
                ...
                tds.each(function (index) {
                    ...
                    if (type == 'text' || type == 'date') {
                    	...
                    	data[index] = $(this).find("input[name='file-select']").data("id");
                    }
                });
                if (options.server) {
                        ...
                        if (mSaveCallback) {
                            ...
                            if (result) {
                                ...
                                tds.each(function (index) {
                                	var type = options.rowTypes[index];
                                	if(type=='file'){
                                		var mInputFileShowHtml=inputFileShowHtml;
                                		if(data[index]&&data[index]!=''){
                                			mInputFileShowHtml=mInputFileShowHtml.replace("${id}",data[index]);
                                			mInputFileShowHtml=mInputFileShowHtml.replace("${name}",$(this).find("input[name='file-select']").val());
                                		}else{
                                			mInputFileShowHtml=mInputFileShowHtml.replace("${id}","");
                                			mInputFileShowHtml=mInputFileShowHtml.replace("${name}","");
                                		}
                                		data[index] = mInputFileShowHtml;
                                    }
                                });
                                row.data(data).draw();
        ...
        this.saveFileCallback = function(saveFileCallback){
        	if (saveFileCallback) {
        		mSaveFileCallback = saveFileCallback;
            }
        }
    };
